            ...
            <el-table-column prop="description" label="操作" width="100">
              <template slot-scope="{ row, $index }">
                <el-button
                  ...
                  size="small"
                  circle
                  @click="edit_user_dialog(row.user)"
                ></el-button>
                <el-button
                  v-if="row.user != 'admin'"
                  type="danger"
                ...
                <el-dialog
                  title="使用者表單-修改"
                  :visible.sync="edit_user_lock"
                >
                  <el-form :model="user_form" label-width="80px">
                    <el-form-item prop="user" label="使用者"
                      ><el-input
                  ...
                  <div slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="edit_user" plain
                      >確定</el-button
                    >
                    <el-button
                      type="danger"
                      @click="edit_user_lock = false"
                      plain
                      >取消</el-button
                    >
                  </div>
            ...
            <el-table-column prop="description" label="操作" width="100">
              <template slot-scope="{ row, $index }">
                ...
                <el-button
                  ...
                  size="small"
                  circle
                  @click="delete_product(row)"
                ></el-button>
                <el-dialog
                  title="商品表單-修改"
                  :visible.sync="edit_product_lock"
                >
                  <el-form :model="product_form" label-width="80px">
                    ...
                    <el-form-item prop="price" label="價格"
                      ...
                      ></el-input
                    ></el-form-item>
                    <el-form-item prop="amount" label="數量"
                      ><el-input
                        v-model="product_form.amount"
                        style="width:300px;"
                        placeholder="請輸入數量"
                      ></el-input
                    ></el-form-item>
                    <el-form-item prop="hot" label="熱門度"
                      ><el-input
                        v-model="product_form.hot"
                        style="width:300px;"
                        placeholder="請輸入熱門度"
                      ></el-input
                    ></el-form-item>
                    <el-form-item prop="time" label="上架時間"
                      ><el-input
                        v-model="product_form.time"
                        style="width:300px;"
                        placeholder="請輸入上架時間"
                      ></el-input
                    ></el-form-item>
                  ...
                  <img
                    :src="product_form.url"
                    style="position: absolute; right: 50px; top: 50px;"
                  />

                  <div slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="edit_product" plain
                      >確定</el-button
                    >
                    <el-button
                      type="danger"
                      @click="edit_product_lock = false"
                      plain
                      >取消</el-button
                    >
                  </div>
...
export default {
  ...
  created() {
    ...
    this.axios.post('/get_all_user', {}).then(response => {
      if (this.user != 'admin')
        this.user_data = response.data.filter(item => {
          return item['user'] == this.user
        })
      else this.user_data = response.data
    })
      ...
      this.axios
        ...
        .then(response => {
          ...
          this.axios.post('/get_all_product_url').then(response => {
            ...
            this.product_data = this.product_data.filter(item => {
              return item['owner'] == this.user
            })
